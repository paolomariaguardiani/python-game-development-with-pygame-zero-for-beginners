...
def draw():
    ...
    screen.draw.textbox(
        question[0].strip(), question_box,  # .strip() to remove accidental extra spaces between the separator "|"
        color="white", shadow=(0.5,0.5),
        scolor="dim gray"
    )
    index = 1
    for answer_box in answer_boxes:
        # strip method removes (strip off) extraspaces in front or behind a string
        screen.draw.textbox(question[index].strip(), answer_box, color="black")  # .strip() to remove accidental extra spaces between the separator "|"
        index = index + 1
...
def on_mouse_down(pos):
    ...
    for box in answer_boxes:
        if box.collidepoint(pos):
            if index is int(question[5]):  # se il numero dell'indice è nel sesto elemento della lista che sarà "2\n" per esempio
                correct_answer()
            else:
                game_over()
        index = index + 1
